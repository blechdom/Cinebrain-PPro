...
$._PPP_={
	...
	keepPanelLoaded : function() {
		app.setExtensionPersistent("com.adobe.Cinebrain", 0); // 0, while testing (to enable rapid reload); 1 for "Never unload me, even when not visible."
	},
	...
	renameFootage : function() {
		...
		if (item) {
			item.name = item.name + ", updated by Cinebrain.";
		} else {
	...
	createSequenceMarkers : function() {
		...
		if (activeSequence) {
			...
			newCommentMarker.name			= 'Marker created by Cinebrain.';
			newCommentMarker.comments		= 'Here are some comments, inserted by Cinebrain.';
			newCommentMarker.end			= 15.6789;

			var newWebMarker				= markers.createMarker(14.345);
			newWebMarker.name				= 'Web marker created by Cinebrain.';
			newWebMarker.comments			= 'Here are some comments, inserted by Cinebrain.';
	...
	mungeXMP : function(){
		...
		if (projectItem) {
			...
			if (ExternalObject.AdobeXMPScript !== undefined) { 	// safety-conscious!
				...
				xmp.setProperty(XMPConst.NS_DM, "scene",	oldSceneVal 	+ " Added by Cinebrain!");
				xmp.setProperty(XMPConst.NS_DM, "creator",	oldDMCreatorVal + " Added by Cinebrain!");
				...
				if(numCreatorValuesPresent > 0) {
					...
					xmp.appendArrayItem(XMPConst.NS_DC, 
										creatorProp,
										"Cinebrain wrote the first value into NS_DC creator field.",
										null, 
										XMPConst.ARRAY_IS_ORDERED);
				}
			...
			if (projectItem) {
				if (projectItem.type == ProjectItemType.CLIP ||	projectItem.type == ProjectItemType.FILE) {
					...
					if (markers) {
						...
						new_marker.name		= 'Marker created by Cinebrain.';
						new_marker.comments	= 'Here are some comments, inserted by Cinebrain.';
		...
		if (app.isDocumentOpen()) {
			...
			if (projectItem) {
				...
				if (ExternalObject.AdobeXMPScript !== undefined) {	// safety-conscious!
					...
					var oldLogValue        			= "";
					var appendThis          		= "This log note inserted by Cinebrain.";
					...
					xmp.setProperty(kPProPrivateProjectMetadataURI, newField, 	"Cinebrain set this, using addPropertyToProjectMetadataSchema().");
					...
					if (foundYet){
						$._PPP_.updateEventPanel("Cinebrain successfully added a field to the project metadata schema, and set a value for it.");
					}
				}
			} else {
				$._PPP_.updateEventPanel("No project items found.");
	...
	onEncoderJobComplete : function (jobID, outputFilePath) {
		...
		eventObj.type	= "com.adobe.csxs.events.CinebrainRenderEvent";
	...
	onProxyTranscodeJobComplete : function (jobID, outputFilePath) {
		...
		var nameToFind	= 'Proxies generated by Cinebrain';
	...
	getPPPInsertionBin : function () {
		var nameToFind = "Here's where Cinebrain puts things.";
				...
				if (newTrackItem){
					...
					if (moComp){
						...
						if (srcTextParam){
							...
							srcTextParam.setValue("New value set by Cinebrain!");
						}
					}
				}
			} else {
	...
	confirmPProHostVersion : function() {
		...
		if (version < 12.1){
			$._PPP_.updateEventPanel("Note: Cinebrain relies on features added in 12.1, but is currently running in " + version + ".");
		}
	},
};
